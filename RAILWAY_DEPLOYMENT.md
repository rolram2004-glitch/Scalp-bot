...
5. In **Variables** impostare:
   ...
   - `PRACTICE_EXECUTION_VARIANT=MAIN` per la NORMALE operativa su OANDA Practice: direzione invariata, TP nominale `+0,03 CHF`, SL nominale `-0,30 CHF`;
...
segnale. La corsia operativa usa TP nominale `+0,03 CHF` e
SL nominale `-0,30 CHF`. La corsia non selezionata resta
...
`-0,30 CHF`. XAUUSD continua a essere bloccato nell'esecuzione OANDA finche il suo modulo
