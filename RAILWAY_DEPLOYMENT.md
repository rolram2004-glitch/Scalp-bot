...
5. In **Variables** impostare:
   ...
   - `PRACTICE_EXECUTION_VARIANT=INVERSE` per la MIRROR operativa su OANDA Practice: `BUY→SELL`, `SELL→BUY`, TP nominale `+0,03 CHF`, SL nominale `-0,15 CHF`;
...
segnale. MIRROR/INVERSE e la corsia operativa e usa TP nominale `+0,03 CHF` e
SL nominale `-0,15 CHF`; il segnale BUY apre SELL e il segnale SELL apre BUY. La corsia MAIN non selezionata resta
...
quotazione. La corsia operativa usa TP nominale `+0,03 CHF` e SL nominale
`-0,15 CHF`. Il generatore dei segnali e il ritmo di scansione restano invariati. XAUUSD continua a essere bloccato nell'esecuzione OANDA finche il suo modulo
