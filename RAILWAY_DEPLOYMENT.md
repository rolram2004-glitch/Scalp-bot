...
5. In **Variables** impostare:
   ...
   - `PRACTICE_EXECUTION_VARIANT=INVERSE` per la MIRROR operativa su OANDA Practice: tutti i segnali forex vengono invertiti una sola volta (`BUY→SELL`, `SELL→BUY`), TP nominale `+2,00 CHF`, SL nominale `-0,20 CHF`;
...
segnale. MIRROR/INVERSE e la corsia operativa e usa TP nominale `+2,00 CHF` e
...
quotazione. La corsia operativa usa TP nominale `+2,00 CHF` e SL nominale
`-0,20 CHF`. Tutti i risultati BUY/SELL degli indicatori forex vengono invertiti una sola volta; il ritmo di scansione resta invariato. XAUUSD continua a essere bloccato nell'esecuzione OANDA finche il suo modulo
