...
Cockpit professionale White Glass di analisi e trading OANDA Practice. Tutti gli indicatori e i setup confluiscono in un solo segnale finale MAIN; la corsia operativa MIRROR/INVERSE lo inverte esattamente una volta: BUY apre SELL e SELL apre BUY. Le protezioni monetarie sono TP `+0,33 CHF` e SL `-2,00 CHF`. MAIN rimane PAPER SHADOW per il confronto. Una sola corsia può essere OANDA e i risultati mantengono separate le valute originali.
...
- `PRACTICE_EXECUTION_VARIANT=INVERSE`: seleziona MIRROR come corsia operativa su OANDA Practice: `BUY→SELL`, `SELL→BUY`, TP nominale `+0,33 CHF`, SL nominale `-2,00 CHF`. Non abilita `OANDA_LIVE`.

La regola operativa non dipende dai colori dell'interfaccia: qualunque segnale finale generato da EMA, RSI, MACD, struttura o dagli altri filtri viene invertito una sola volta dalla corsia MIRROR (`BUY→SELL`, `SELL→BUY`), con TP nominale `+0,33 CHF` e SL nominale `-2,00 CHF`. MAIN conserva il verso originale come PAPER SHADOW. Il ritmo di scansione resta invariato. Il P&L delle due corsie viene misurato separatamente con bid/ask reali, quindi spread e slippage non vengono nascosti.
