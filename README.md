...
Cockpit professionale White Glass di analisi e trading OANDA Practice. Tutti gli indicatori e i setup confluiscono in un solo segnale finale MAIN e la corsia operativa lo esegue nella stessa direzione: BUY resta BUY e SELL resta SELL. Le protezioni monetarie sono TP `+0,10 CHF` e SL `-0,20 CHF`. MIRROR/INVERSE rimane PAPER SHADOW per il confronto. Una sola corsia può essere OANDA e i risultati mantengono separate le valute originali.
...
- `PRACTICE_EXECUTION_VARIANT=MAIN`: seleziona MAIN come corsia operativa su OANDA Practice: `BUY→BUY`, `SELL→SELL`, TP nominale `+0,10 CHF`, SL nominale `-0,20 CHF`. Non abilita `OANDA_LIVE`.

La regola operativa non dipende dai colori dell'interfaccia: qualunque segnale finale generato da EMA, RSI, MACD, struttura o dagli altri filtri viene eseguito una sola volta dalla corsia MAIN (`BUY→BUY`, `SELL→SELL`), con TP nominale `+0,10 CHF` e SL nominale `-0,20 CHF`. MIRROR conserva il verso opposto come PAPER SHADOW. Il ritmo di scansione resta invariato. Il P&L delle due corsie viene misurato separatamente con bid/ask reali, quindi spread e slippage non vengono nascosti.
...
TP `+0,10 CHF` e SL `-0,20 CHF`. Prezzi reali, riconciliazione e SL/TP restano obbligatori. PAPER resta a
