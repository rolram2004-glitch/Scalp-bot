...
function LaneCard({ variant, lane, symbol, executionReady }: { variant: 'MAIN' | 'INVERSE'; lane?: SignalLaneSnapshot; symbol: string; executionReady: boolean }) {
  ...
  return (
    <article className={`pro-lane-card ${variant.toLowerCase()}`}>
      ...
      <p>{selected ? variant === 'MAIN' ? 'MAIN OPERATIVA: direzione normale invariata, TP nominale +0,10 CHF, SL nominale -0,60 CHF. ' : 'MIRROR OPERATIVA: direzione opposta, TP nominale +0,10 CHF, SL nominale -0,60 CHF. ' : ''}{lane?.reasoning || 'Nessuno snapshot OANDA disponibile.'}</p>
    </article>
  );
}
  ...
  return (
    <div className="pro-setup">
      ...
      <section className="pro-duel">
        ...
        <div className="pro-duel-rule"><span>STESSO SNAPSHOT</span><strong>VS</strong><p><b>MAIN/NORMALE</b> invia OANDA senza invertire BUY/SELL.<br />TP nominale +0,10 CHF · SL nominale -0,60 CHF; MIRROR resta PAPER SHADOW.</p><Link to="/vs">APRI CONFRONTO COMPLETO</Link></div>
