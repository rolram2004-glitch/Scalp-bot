...
function LaneCard({ variant, lane, symbol, executionReady }: { variant: 'MAIN' | 'INVERSE'; lane?: SignalLaneSnapshot; symbol: string; executionReady: boolean }) {
  ...
  return (
    <article className={`pro-lane-card ${variant.toLowerCase()}`}>
      ...
      <p>{selected ? variant === 'MAIN' ? 'MAIN OPERATIVA: direzione normale invariata, TP nominale +0,60 CHF, SL nominale -0,10 CHF. ' : 'MIRROR OPERATIVA: direzione opposta, TP nominale +0,60 CHF, SL nominale -0,10 CHF. ' : ''}{lane?.reasoning || 'Nessuno snapshot OANDA disponibile.'}</p>
    </article>
  );
}
    ...
    <div className="pro-setup">
      <section className="pro-setup-hero">
        <div className="pro-hero-brand">
          ...
          <p>Modalità NORMALE ULTRA: il segnale BUY apre BUY e SELL apre SELL. Scansione ogni 1s; massimo 100 ingressi al minuto e 15.000 al giorno, senza limite di perdita giornaliera su OANDA Practice. Protezioni fisse: TP +0,60 CHF e SL -0,10 CHF. Size massima 1.000 unità, ridotta automaticamente soltanto quando lo spread lo richiede.</p>
        </div>
      ...
      <section className="pro-duel">
        ...
        <div className="pro-duel-rule"><span>STESSO SNAPSHOT</span><strong>VS</strong><p><b>MAIN/NORMALE</b> invia OANDA senza invertire BUY/SELL.<br />TP nominale +0,60 CHF · SL nominale -0,10 CHF; MIRROR resta PAPER SHADOW.</p><Link to="/vs">APRI CONFRONTO COMPLETO</Link></div>
