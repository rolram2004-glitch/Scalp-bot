...
function LaneCard({ variant, lane, symbol, executionReady }: { variant: 'MAIN' | 'INVERSE'; lane?: SignalLaneSnapshot; symbol: string; executionReady: boolean }) {
  ...
  return (
    <article className={`pro-lane-card ${variant.toLowerCase()}`}>
      ...
      <p>{variant === 'INVERSE' ? 'MIRROR OPERATIVA: direzione opposta, TP nominale +0,20 CHF, SL nominale -1,20 CHF. ' : ''}{lane?.reasoning || 'Nessuno snapshot OANDA disponibile.'}</p>
    </article>
  );
}
  ...
  return (
    <div className="pro-setup">
      ...
      <section className="pro-duel">
        ...
        <div className="pro-duel-rule"><span>STESSO SNAPSHOT</span><strong>VS</strong><p><b>Una sola corsia</b> invia OANDA.<br /><b>MIRROR</b>: direzione opposta · TP nominale +0,20 CHF · SL nominale -1,20 CHF.</p><Link to="/vs">APRI CONFRONTO COMPLETO</Link></div>
