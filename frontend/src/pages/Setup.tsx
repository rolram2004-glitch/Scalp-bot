...
function Lane({ lane, executionReady, symbol }: { lane?: SignalLaneSnapshot; executionReady: boolean; symbol: string }) {
  ...
  return (
    <article className={`lane-command ${lane.variant.toLowerCase()} ${lane.selectedForExecution ? 'selected' : ''}`}>
      ...
      <p>{lane.selectedForExecution ? lane.variant === 'MAIN' ? 'MAIN OPERATIVA · direzione normale invariata · TP nominale +0,20 CHF · SL nominale -2,00 CHF. ' : 'MIRROR OPERATIVA · TUTTI I SEGNALI INVERTITI UNA VOLTA · BUY→SELL, SELL→BUY · TP nominale +0,20 CHF · SL nominale -2,00 CHF. ' : ''}{lane.reasoning || 'N/A'}</p>
    </article>
  );
}
