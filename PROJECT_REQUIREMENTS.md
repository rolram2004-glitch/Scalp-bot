...
- La pagina `VS` deve mantenere MAIN e MIRROR (INVERSE) in corsie separate. MIRROR e la corsia operativa, applica TP nominale `+0,20 CHF` e SL nominale `-2,00 CHF` e inverte una sola volta il segnale finale proveniente da qualunque setup o indicatore forex (`BUY→SELL`, `SELL→BUY`); MAIN conserva la direzione originale come PAPER SHADOW. Il precedente TP `2,00 CHF` diventa SL e il precedente SL `0,20 CHF` diventa TP. Ogni PAPER SHADOW deve nascere soltanto dopo l'apertura verificata della corsia OANDA corrispondente. Il confronto primario usa l'unita normalizzata `R`; CHF, JPY, USD e altre valute restano esposte separatamente e non vengono mai sommate.
...
- Forex MAIN conserva il proprio piano di analisi. La MIRROR operativa usa
  la direzione opposta (`BUY→SELL`, `SELL→BUY`), TP nominale `+0,20 CHF` e SL nominale `-2,00 CHF`, calcolati
