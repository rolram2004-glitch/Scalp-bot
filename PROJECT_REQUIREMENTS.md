...
- La pagina `VS` deve mantenere MAIN e MIRROR (INVERSE) in corsie separate. MAIN e la corsia operativa, applica TP nominale `+0,10 CHF` e SL nominale `-0,20 CHF` ed esegue una sola volta il segnale finale proveniente da qualunque setup o indicatore forex (`BUY→BUY`, `SELL→SELL`); MIRROR conserva la direzione opposta come PAPER SHADOW. Ogni PAPER SHADOW deve nascere soltanto dopo l'apertura verificata della corsia OANDA corrispondente. Il confronto primario usa l'unita normalizzata `R`; CHF, JPY, USD e altre valute restano esposte separatamente e non vengono mai sommate.
...
- Forex MAIN conserva il proprio piano di analisi ed e la corsia operativa: usa
  la direzione originale (`BUY→BUY`, `SELL→SELL`), TP nominale `+0,10 CHF` e SL nominale `-0,20 CHF`, calcolati
