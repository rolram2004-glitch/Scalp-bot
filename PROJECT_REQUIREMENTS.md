...
- La pagina `VS` deve mantenere MAIN e MIRROR (INVERSE) in corsie separate. La MIRROR operativa usa lo stesso Signal ID, inverte BUY/SELL e applica TP nominale `+0,20 CHF` e SL nominale `-0,60 CHF`. Ogni PAPER SHADOW deve nascere soltanto dopo l'apertura verificata della corsia OANDA corrispondente. Il confronto primario usa l'unita normalizzata `R`; CHF, JPY, USD e altre valute restano esposte separatamente e non vengono mai sommate.
