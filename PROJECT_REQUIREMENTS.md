...
- La pagina `VS` deve mantenere MAIN e MIRROR (INVERSE) in corsie separate. MIRROR e la corsia operativa, applica TP nominale `+0,20 CHF` e SL nominale `-0,60 CHF` e inverte la direzione (`BUY→SELL`, `SELL→BUY`); MAIN conserva la direzione del segnale come PAPER SHADOW. Ogni PAPER SHADOW deve nascere soltanto dopo l'apertura verificata della corsia OANDA corrispondente. Il confronto primario usa l'unita normalizzata `R`; CHF, JPY, USD e altre valute restano esposte separatamente e non vengono mai sommate.
...
- Forex MAIN conserva il proprio piano di analisi. La MIRROR operativa usa la
  direzione opposta (`BUY→SELL`, `SELL→BUY`), TP nominale `+0,20 CHF` e SL nominale `-0,60 CHF`, calcolati
