...
const fixedCashCases = [
  {
    ...
    expected: {
      BUY: { stopLoss: "0.89990", takeProfit: "0.90035" },
      SELL: { stopLoss: "0.90020", takeProfit: "0.89975" }
    }
  },
  {
    ...
    expected: {
      BUY: { stopLoss: "1.09988", takeProfit: "1.10037" },
      SELL: { stopLoss: "1.10022", takeProfit: "1.09973" }
    }
  },
  {
    ...
    directFactors: undefined,
    expected: {
      BUY: { stopLoss: "159.200", takeProfit: "159.275" },
      SELL: { stopLoss: "159.264", takeProfit: "159.189" }
    }
  }
];
...
for (const cashCase of fixedCashCases) {
  for (const strategyVariant of ["MAIN", "INVERSE"]) {
    for (const side of ["BUY", "SELL"]) {
      test(`${strategyVariant} ACCOUNT_CASH ${side} fixes SL 0.20 CHF and TP 0.25 CHF using ${cashCase.label}`, async () => {
      ...
      assert.ok(
        Math.abs(actual.reward - 0.25) <= cashRoundingTolerance(
          cashCase.displayPrecision,
          cashCase.expectedUnits,
          cashCase.gainFactor
        )
...
for (const side of ["BUY", "SELL"]) {
  test(`ACCOUNT_CASH ${side} reduces units when needed and preserves TP 0.25 / SL 0.20 CHF`, async () => {
    ...
    assert.ok(Math.abs(result.trade.rewardAmount - 0.25) <= cashRoundingTolerance(5, expectedUnits, 0.91));
  });
}
...
test("MAIN ACCOUNT_CASH recalculates protection from the verified post-fill entry", async () => {
  ...
  const actual = cashAtProtection(1.10020, "1.09995", "1.10050", 1000, 0.8, 0.82);
  assert.ok(Math.abs(result.trade.riskAmount - actual.risk) < 1e-10);
  assert.ok(Math.abs(result.trade.rewardAmount - actual.reward) < 1e-10);
});
...
test("MAIN ACCOUNT_CASH rejects any units, cash target, or explicit-price override outside its fixed contract", async () => {
  for (const [overrides, reason] of [
    ...
    [{ rewardAmount: 0.24 }, "ACCOUNT_CASH_TARGETS_INVALID"],
