...
const fixedCashCases = [
  {
    ...
    expected: {
      BUY: { stopLoss: "0.89990", takeProfit: "0.90014" },
      SELL: { stopLoss: "0.90020", takeProfit: "0.89996" }
    }
  },
  {
    ...
    expected: {
      BUY: { stopLoss: "1.09990", takeProfit: "1.10014" },
      SELL: { stopLoss: "1.10020", takeProfit: "1.09996" }
    }
  },
  {
    ...
    directFactors: undefined,
    expected: {
      BUY: { stopLoss: "159.208", takeProfit: "159.238" },
      SELL: { stopLoss: "159.256", takeProfit: "159.226" }
    }
  }
];
...
for (const cashCase of fixedCashCases) {
  for (const strategyVariant of ["MAIN", "INVERSE"]) {
    for (const side of ["BUY", "SELL"]) {
      test(`${strategyVariant} ACCOUNT_CASH ${side} fixes SL 0.15 CHF and TP 0.03 CHF using ${cashCase.label}`, async () => {
      ...
      assert.ok(
        Math.abs(actual.risk - 0.15) <= cashRoundingTolerance(
      ...
      assert.ok(
        Math.abs(actual.reward - 0.03) <= cashRoundingTolerance(
          cashCase.displayPrecision,
          cashCase.expectedUnits,
          cashCase.gainFactor
        )
...
for (const side of ["BUY", "SELL"]) {
  test(`ACCOUNT_CASH ${side} reduces units when needed and preserves TP 0.03 / SL 0.15 CHF`, async () => {
    const expectedUnits = 208;
    ...
    assert.equal(calls.replaceTradeDependentOrders, 1);
    assert.ok(Math.abs(result.trade.riskAmount - 0.15) <= cashRoundingTolerance(5, expectedUnits, 0.9));
    assert.ok(Math.abs(result.trade.rewardAmount - 0.03) <= cashRoundingTolerance(5, expectedUnits, 0.91));
  });
}
...
test("MAIN ACCOUNT_CASH recalculates protection from the verified post-fill entry", async () => {
  ...
  const actual = cashAtProtection(1.10020, "1.10001", "1.10024", 1000, 0.8, 0.82);
  assert.ok(Math.abs(result.trade.riskAmount - actual.risk) < 1e-10);
  assert.ok(Math.abs(result.trade.rewardAmount - actual.reward) < 1e-10);
});
...
test("MAIN ACCOUNT_CASH rejects any units, cash target, or explicit-price override outside its fixed contract", async () => {
  for (const [overrides, reason] of [
    ...
    [{ riskAmount: 0.14 }, "ACCOUNT_CASH_TARGETS_INVALID"],
    [{ rewardAmount: 0.02 }, "ACCOUNT_CASH_TARGETS_INVALID"],
