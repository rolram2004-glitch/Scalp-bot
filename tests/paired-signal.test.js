...
test("OANDA_DEMO INVERSE ACCOUNT_CASH maps BUY to SELL and SELL to BUY with SL 2.00 / TP 0.33 CHF", () => {
  const wideMarket = {
    ...market,
    time: new Date().toISOString(),
    bid: 1.1,
  ...
  for (const [mainAction, expectedOrderAction] of [["BUY", "SELL"], ["SELL", "BUY"]]) {
    ...
    assert.ok(Math.abs(result.inverse.riskRewardRatio - (0.33 / 2)) < 1e-12);
    ...
    assert.match(result.inverse.reasoning, /TP nominale \+0\.33 CHF, SL nominale -2\.00 CHF/);
  }
});
...
test("OANDA_DEMO MAIN ACCOUNT_CASH keeps the normal direction and defers fixed CHF protection to the fill", () => {
  ...
  assert.ok(Math.abs(result.main.riskRewardRatio - (0.33 / 2)) < 1e-12);
  assert.equal(result.executionBlockedReason, undefined);
  assert.match(result.main.reasoning, /NORMALE sul segnale: BUY resta BUY/);
  assert.match(result.main.reasoning, /TP nominale \+0\.33 CHF, SL nominale -2\.00 CHF/);
});

test("OANDA_DEMO MAIN ACCOUNT_CASH keeps SELL as SELL with TP 0.33 CHF and SL 2.00 CHF", () => {
  const result = snapshot({
    tradingMode: "OANDA_DEMO",
    liveExecutionVariant: "MAIN",
    executionGateVerified: true,
